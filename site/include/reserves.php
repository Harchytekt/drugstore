<?php
    $isReserveEmpty = False;
    if ($current_id == 1) {
        $reponse = $bd->prepare("SELECT username, user_id, name, medicine_id, dosage
            FROM Medicines NATURAL JOIN Lists NATURAL JOIN Users WHERE user_id != 1 ORDER BY user_id, name;");
        $reponse->execute();
    } else {
        $reponse = $bd->prepare("SELECT medicine_id FROM Medicines
            NATURAL JOIN Lists NATURAL JOIN Users WHERE user_id = $current_id;");
        $reponse->execute();
        $test = $reponse->fetch();
        if (count($test) == 1) {
            $isReserveEmpty = True;
            ...
            $reponse = $bd->prepare("SELECT username, user_id, name, medicine_id, dosage
                FROM Medicines NATURAL JOIN Lists NATURAL JOIN Users WHERE user_id = $current_id ORDER BY user_id, name;");
            $reponse->execute();
        }
    }
    ...
    if ($isReserveEmpty) {
        ?>
        <h2 id="emptyRes">Votre réserve est vide</h2>
        <?php
    } else {
        ?>
